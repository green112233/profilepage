package com.sunjong.home.controller;
...
import com.sunjong.home.dao.MemberDao;
import com.sunjong.home.dto.MemberDto;
...
public class MemberController {
	...
	public String joinOk(HttpServletRequest request,Model model) {
		...
		//checkFlag 가 0 이면 회원가입 가능 1이면 불가능
		if (checkFlag == 0) {
	...
	public String joinOk(HttpServletRequest request,Model model,HttpSession session) {
		...
		int checkFlag = dao.idCheckDao(request.getParameter("mid"),request.getParameter("mpw"));
		if(CheckFlag==1) {
			session.setAttribute("sessionId",request.getParameter("mid"));
			//로그인 성공시 세션에 현재 로그인한 아이디를 저장
			MemberDto = dao.memberInfoDao(request.getParameter("mid"));
	...
	public String logout(HttpSession session,HttpServletResponse response) throws IOException{
		...
		response.setContentType("text/html;charset=utf-8"); //utf-8로 경고창에 출력될 문자셋 세팅
		response.setChracterEncoding("utf-8");
